...
class Scene:
    ...
    def create_axes(self) -> Self:
        ...
        axis_x.rotation_euler = (radians(0), radians(0), radians(-90))
        ...
        axis_z.rotation_euler = (radians(90), radians(0), radians(0))
        ...
        if self.config_axes.label_x:
            ...
            axis_label = bpy.data.objects.new(
                name="axis label x", object_data=font_curve
            )
            ...
            z = self.shift[2]
            if self.config_axes.enable_ticks and len(self.config_axes.ticks_x) > 0:
                y += self.config_axes.tick_length + self.config_axes.font_size_ticks
            axis_label.location = (x, y, z)
            axis_label.rotation_euler = (radians(0), radians(0), radians(180))
        ...
        if self.config_axes.label_y:
            ...
            font_curve.align_y = "TOP"
            axis_label = bpy.data.objects.new(
                name="axis label y", object_data=font_curve
            )
            col.objects.link(axis_label)
            axis_label.data.font = font
            x = (
            ...
            z = self.shift[2]
            if self.config_axes.enable_ticks and len(self.config_axes.ticks_y) > 0:
                x += self.config_axes.tick_length + self.config_axes.font_size_ticks
            axis_label.location = (x, y, z)
            axis_label.rotation_euler = (radians(0), radians(0), radians(90))
        ...
        if self.config_axes.label_z:
            ...
            font_curve.align_y = "TOP"
            axis_label = bpy.data.objects.new(
                name="axis label z", object_data=font_curve
            )
            col.objects.link(axis_label)
            axis_label.data.font = font
            x = (
            ...
            if self.config_axes.enable_ticks and len(self.config_axes.ticks_z) > 0:
                x += self.config_axes.tick_length + self.config_axes.font_size_ticks
            axis_label.location = (x, y, z)
            axis_label.rotation_euler = (radians(0), radians(90), radians(90))

        if self.config_axes.enable_ticks:
            # tick mesh
            ...
            for [idx, label] in enumerate(self.config_axes.ticks_x):
                ...
                x = (
                    self.shift[0]
                    + self.size_bounding_box * idx / (N_x - 1)
                ...
                tick_label.rotation_euler = (radians(0), radians(0), radians(180))
            ...
            for [idx, label] in enumerate(self.config_axes.ticks_y):
                ...
                y = (
                    self.shift[1]
                    + self.size_bounding_box * idx / (N_y - 1)
                    + 0.5 * self.config_axes.line_width
                )
                z = self.shift[2]
                align_x = "CENTER"
                ...
                if idx == N_y - 1:
                    ...
                    align_x = "RIGHT"
                tick.location = (x, y, z)
                tick.rotation_euler = (radians(0), radians(0), radians(-90))
                ...
                tick_label.rotation_euler = (radians(0), radians(0), radians(90))
            ...
            for [idx, label] in enumerate(self.config_axes.ticks_z):
                ...
                z = (
                    self.shift[2]
                    + self.size_bounding_box * idx / (N_z - 1)
                    + 0.5 * self.config_axes.line_width
                )
                align_x = "CENTER"
                if idx == 0:
                ...
                if idx == N_z - 1:
                    ...
                    align_x = "LEFT"
                tick.location = (x, y, z)
                tick.rotation_euler = (radians(90), radians(90), radians(0))
                ...
                tick_label.rotation_euler = (radians(0), radians(90), radians(90))
    ...
    def create_camera(self, view: str = "3D") -> Self:
        ...
        if view == "3D":
            ...
            bpy.data.objects["Camera"].rotation_euler = (radians(90), 0, radians(150))
        elif view == "2D":
            bpy.data.objects["Camera"].location = (5, 40, 15)
            bpy.data.objects["Camera"].rotation_euler = (radians(90), 0, radians(180))
    ...
    def create_cylinders(
        ...
        for no in track(range(len(r)), description="Creating cylinder geometries"):
            ...
            col.objects.link(obj)
        for obj in track(col.objects, description="Creating cylinder materials"):
            obj.data.materials.append(mat.copy())
            obj.data.materials[0].name = obj.name
    ...
    def create_lights(self) -> Self:
        ...
        obj.data.energy = 9
        obj.data.angle = radians(10)
        obj.rotation_euler = (radians(-30), 0, radians(-70))
    ...
    def create_solid(self, pth: Path, style: str = "SOLID_DEFAULT"):
        ...
        obj.rotation_euler = (0, 0, radians(0))
        obj.scale = (self.scale, self.scale, self.scale)
        obj.location = self.shift
        ...
        # S = Matrix.Rotation(radians(90), 4, "Z").to_4x4()
    ...
    def create_spheres(
        ...
        for no in track(range(len(r)), description="Creating sphere geometries"):
            obj = bpy.data.objects.new(f"sphere {no}", obj_template.data.copy())
            obj.location = pos[no, :]
            obj.scale = (r[no], r[no], r[no])
            col.objects.link(obj)
        for obj in track(col.objects, description="Creating sphere materials"):
            obj.data.materials.append(mat.copy())
            obj.data.materials[0].name = obj.name
        bpy.data.objects.remove(obj_template)
        bpy.context.scene.collection.children.link(col)
    ...
    def create_void(self, pth: Path, style: str = "ICE") -> Self:
        """
        ...
        obj.location = self.shift
        obj.data.materials.clear()
        obj.data.materials.append(self.get_material(style, "void"))
        self.has_void = True
        return self
    ...
    def get_material(self, style: str, name: str):
        ...
        if mat:
            ...
            links.new(node.outputs[0], output.inputs[0])

            if style in ["STRUCTURE_DEFAULT", "CLUSTER_DEFAULT"]:
                node.inputs[0].default_value = Color("#58646E").lnrgba
                node.inputs[1].default_value = 0.4  # metallic
                node.inputs[2].default_value = 0.5  # roughness
                node.inputs[7].default_value = 0.1  # subsurface ratio
                node.inputs[8].default_value = Color("#D5D8D7").lnrgb  # subsurf
                node.inputs[12].default_value = 0.1  # specular
                node.inputs[23].default_value = 0.1  # sheen weight
                node.inputs[24].default_value = 0  # sheen roughness
            elif style == "SOLID_DEFAULT":
                node.inputs[0].default_value = Color("#947C5E").lnrgba
                node.inputs[1].default_value = 0  # metallic
                node.inputs[2].default_value = 1  # roughness
                node.inputs[12].default_value = 0.5  # specular
            elif style == "BACTERIA_GREEN":
                node.inputs[0].default_value = Color("#3F901A").lnrgba
                node.inputs[1].default_value = 0.2  # metallic
                node.inputs[2].default_value = 1  # roughness
                node.inputs[12].default_value = 0  # specular
            elif style.startswith("ICE"):
                if "ICE_TILED" == style:
                    color = choice(
                        ["#7891e3", "#92b2e8", "#95c5e8", "#b7d2e7", "#d6edff"]
                    )
                    node.inputs[0].default_value = Color(color).lnrgba
                else:
                    node.inputs[0].default_value = Color("#85A2E6").lnrgba
                node.inputs[1].default_value = 0  # metallic
                node.inputs[2].default_value = 1  # roughness
                node.inputs[7].default_value = 0.1  # subsurface ratio
                node.inputs[12].default_value = 1  # specular
                node.inputs[23].default_value = 0  # sheen weight
                node.inputs[24].default_value = 1  # sheen roughness
            elif style == "AXES":
                node.inputs[0].default_value = Color("#0001").lnrgba
                node.inputs[1].default_value = 0  # metallic
                node.inputs[2].default_value = 1  # roughness
                node.inputs[12].default_value = 0  # specular
            return mat
